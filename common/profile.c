...
BOOL profile_enum(const char *filename, void *arg,
                  BOOL (*proc)(void *arg, const char *para, const char *key, const char *data)) {
	TEXTFILEH fh;
	BOOL r;
...
static BOOL seakey(PFILEH hdl, PFPOS *pfp, const char *app, const char *key) {
	...
	while (size) {
		...
		} else if ((ret.apphit) && (len > ret.keylen) && (!milstr_memcmp(buf, key))) {
			buf += ret.keylen;
			len -= ret.keylen;
			buf = delspace(buf, &len);
			if ((len) && (buf[0] == '=')) {
...
BOOL profile_read(const char *app, const char *key, const char *def, char *ret, UINT size,
                  PFILEH hdl) {
	PFPOS pfp;
...
BOOL profile_write(const char *app, const char *key, const char *data, PFILEH hdl) {
	PFPOS pfp;
	UINT newsize;
	UINT datalen;
	char *buf;

	if ((hdl == NULL) || (hdl->flag & PFILEH_READONLY) || (data == NULL) ||
	    (seakey(hdl, &pfp, app, key) != SUCCESS)) {
		return (FAILURE);
	}
	if (!pfp.apphit) {
...
void profile_iniread(const char *path, const char *app, const PFTBL *tbl, UINT count, PFREAD cb) {
	PFILEH pfh;
	const PFTBL *p;
	const PFTBL *pterm;
	char work[512];
	...
	pterm = tbl + count;
	while (p < pterm) {
		if (profile_read(app, p->item, NULL, work, sizeof(work), pfh) == SUCCESS) {
			...
			case PFTYPE_BITMAP:
				bitmapset((BYTE *)p->value, p->arg, (!milstr_cmp(work, str_true)) ? TRUE : FALSE);
				break;
...
void profile_iniwrite(const char *path, const char *app, const PFTBL *tbl, UINT count, PFWRITE cb) {
	PFILEH pfh;
	const PFTBL *p;
	const PFTBL *pterm;
	const char *set;
	...
	while (p < pterm) {
		if (!(p->itemtype & PFFLAG_RO)) {
			...
			case PFTYPE_BITMAP:
				set = (bitmapget((BYTE *)p->value, p->arg)) ? str_true : str_false;
				break;
