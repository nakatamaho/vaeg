...
#if defined(RESIZE_FASTCOPY)
static void fastcopyfunc(RSZHDL hdl, UINT type, BYTE *dst, int dalign, const BYTE *src,
                         int salign) {
	UINT height;
	FNCNV cnv;
...
static void aaout16(RSZEX hdl, BYTE *dst) {
	...
	do {
		...
		col = (tmp >> (MXBITS + MYBITS - B16SFT)) & (((1 << B16BIT) - 1) << B16SFT);
		tmp = buf[1] + (buf[1] >> (8 - G16BIT));
		col += (tmp >> (MXBITS + MYBITS - G16SFT)) & (((1 << G16BIT) - 1) << G16SFT);
		tmp = buf[2] + (buf[2] >> (8 - R16BIT));
		col += (tmp >> (MXBITS + MYBITS - R16SFT)) & (((1 << R16BIT) - 1) << R16SFT);
		*(UINT16 *)dst = (UINT16)col;
		dst += 2;
		buf += 3;
	} while (--rem);
...
static void aaout16by24(RSZEX hdl, BYTE *dst) {
	...
	rem = hdl->width;
	do {
		col = (buf[0] >> (MXBITS + MYBITS + 8 - B16BIT - B16SFT)) & (((1 << B16BIT) - 1) << B16SFT);
		col +=
		    (buf[1] >> (MXBITS + MYBITS + 8 - G16BIT - G16SFT)) & (((1 << G16BIT) - 1) << G16SFT);
		col +=
		    (buf[2] >> (MXBITS + MYBITS + 8 - R16BIT - R16SFT)) & (((1 << R16BIT) - 1) << R16SFT);
		*(UINT16 *)dst = (UINT16)col;
		dst += 2;
		buf += 3;
	} while (--rem);
...
static const AAMIX fnaamix[RSZFNMAX] = {aamix8, aamix16, aamix24, aamix24, aamix16};
static const AAOUT fnaaout[RSZFNMAX] = {aaout8, aaout16, aaout24, aaout16by24, aaout24by16};

static void areaavefunc(RSZEX hdl, UINT type, BYTE *dst, int dalign, const BYTE *src, int salign) {
	AAMIX aamix;
	AAOUT aaout;
	UINT posy;
	UINT i;
