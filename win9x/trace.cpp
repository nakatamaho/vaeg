...
#if defined(EXT)
#define	VIEW_BUFFERSIZE	48000		//20488
#define VIEW_DROPPEDSIZE 24000		//16384		// < VIEW_BUFFERSIZE
#define VIEW_TIMER	1
#define VIEW_DELAY	100				// (msec)
#else
...
#if defined(EXT)
static const int	crlflen = 2;			// strlen(crlf)
#endif
...
static void starttimer(void) {
	if (!tracewin.timerrunning) {
		tracewin.timerrunning = TRUE;
		if (!SetTimer(tracewin.hwnd , VIEW_TIMER, VIEW_DELAY, NULL)) {
			tracewin.timerrunning = FALSE;
		}
	}
}

static void killtimer(void) {
	KillTimer(tracewin.hwnd, VIEW_TIMER);
	tracewin.timerrunning = FALSE;
}
#endif
...
#if defined(EXT)
	SendMessage(hWnd, EM_LINESCROLL, 0, MaxPos);
#else
...
#if defined(EXT)
static void View_DoWrite(void) {
	SendMessage(hView, WM_SETREDRAW, FALSE, 0);
	SetWindowText(hView, szView);
	View_ScrollToBottom(hView);
	SendMessage(hView, WM_SETREDRAW, TRUE, 0);
}
#endif
...
#if defined(EXT)
	vlen = tracewin.viewused;
#else
	vlen = strlen(szView);
#endif
	if ((vlen + len + 3) > VIEW_BUFFERSIZE) {
/*
		p = szView + vlen - (VIEW_DROPPEDSIZE - len - 3);
		vlen = VIEW_DROPPEDSIZE - len - 3;
*/
		p = szView;
		while(*p) {
			vlen--;
#if defined(EXT)
			if ((*p++ == 0x0a) && ((vlen + len + 3) <= VIEW_DROPPEDSIZE)) {
#else
...
#if defined(EXT)
		memmove(szView, p, vlen + 1);
		tracewin.viewused = vlen;
#else
		strcpy(szView, p);
#endif
	}
#if defined(EXT)
	strcpy(szView + vlen, lpszString);
	strcpy(szView + vlen + len, crlf);
	tracewin.viewused += len + crlflen;
#else
...
#if defined(EXT)
		{
#define KEYWORDS_MAX 5
			BOOL usefilter = FALSE;
			static const char *keywords[KEYWORDS_MAX]={
				"keyboard:",
/*
				"SGP:",
				"cpu: int 0x87",
...
*/
			...
			if (usefilter) {
				for (i = 0; i < KEYWORDS_MAX; i++) {
					if (keywords[i] != NULL) {
						if (strstr(buf, keywords[i])) break;
					}
				}
				if (i == KEYWORDS_MAX) return;
			}
		}
#endif
#if defined(LINEHEADER)
		sprintf(linebuf, "%05u: %s", tracewin.lineno, buf);
		tracewin.lineno++;
		tracewin.lineno %= 100000;
		if ((tracewin.en & 4) && (hView)) {
			View_AddString(linebuf);
		}
		if (tracewin.fh != FILEH_INVALID) {
			trfh_add(linebuf);
			trfh_add(crlf);
