...
■ 1. 概要
  ...
  本ソフトウェアはPC-9801エミュレータNeko Project II (以降、np2)をベースに
  開発されています。
  以降では、88VA Eternal Grafx (以降、VA-EG) 固有の事項のみ説明します。
  それ以外については、後半に添付したnp2の説明書を参照してください。
...
・最低動作環境
    ...
    画面の色として16ビット, 24ビット, 32ビットのいずれか表示可能なこと
    (24ビットは未確認)
    Windowsキーのある日本語キーボード
...
■ 3. 使用方法
  ...
  バージョンアップボードのエミュレートをする場合、バージョンアップボードの
  ROMイメージが必要です。ROMイメージの作成には GETROMVA (r070422以降)を使用
  してください。
...
・実機用ツール
  ...
  ROMイメージ作成
    ・GETROMVA.EXE (VAEGTOOL.LZH内) で作成できます。
      r060407以降を使用してください。
      バージョンアップボードをエミュレートする場合はr070422以降を使用してく
      ださい。
    ・XDISK2VA で作成できます。VAとWindowsマシンをRS-232Cリバースケーブルで
      接続することで、VAからWindowsにROMイメージを転送します。
  フロッピーディスクイメージ作成
    ・XDISK2VA で作成できます。VAとWindowsマシンをRS-232Cリバースケーブルで
      接続することで、VAのFDDからデータを読み出しWindows上にディスクイメージ
      を作成します。
    なお、PC-9801上でMAKE_HDを使用するなど、他の方法で作成した
    D88形式, XDF(ベタイメージ)形式のイメージも利用可能です。
...
■ 7. 使用許諾条件
    ...
    Neko Project II に準じます。

   ・個人的使用においては制限なく使用することができます。
   ・ソースコードは 修正BSDライセンスとします。

  なお、本プログラムを使用したために何らかの損害が生じたとしても、製作者は
  一切責任を負いません。使用者の責任において使用するようにしてください。


■ 8. 謝辞

  本ソフトウェアには、PC-9801エミュレータ Neko Project II のソースを全面的に
  使用させていただいています。
    Neko Project II (c) NP2 developer team, 1999-2001,2003,2004

  本ソフトウェアのZ80エミュレーション処理には、PC-8801エミュレータM88のソース
  を使用させていただいています。
    M88 - PC8801 Series Emulator
    Copyright (C) by cisc 1998, 2002.

  各ソフトウェアの作者の方々に心より感謝いたします。


■ 9. 連絡先

  本ソフトウェアについてのお問い合わせは以下へお願いします。
  極力Webでお願いします。
  Neko Project IIの作者様への問い合わせはおやめください。

    ・Web
      http://www.pc88.gr.jp/vaeg/

    ・メール
      shinra＊keb_biglobe_ne_jp 
      (メールアドレスの ＊ は @ に、 _ は . に置き換えてお読みください。)

        なお、上記メールアドレスは将来変更する可能性もあります。その場合は、
      以下にメールすることで最新のアドレスが自動返信されます。
      (そのようにするつもりです。)
        shinra＊mtj_biglobe_ne_jp




以下、np2 v0.80のREADME.TXTです。
(メールアドレスの部分のみ書き換えています)
==============================================================================
　Neko Project II
