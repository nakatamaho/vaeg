...
#if defined(SUPPORT_PC88VA)
static	char		szClassName[] = "VAEG-MainWindow";
#else
static	char		szClassName[] = "NP2-MainWindow";
		...
		BYTE		np2break = 0;									// ver0.30
										// 各bitの意味はNP2BREAK_MAIN, NP2BREAK_DEBUG
		BOOL		winui_en;
...
static	BYTE		np2stopemulate = 0;
										// 0..通常実行
										// 1..停止 次のメッセージを受け取るまでsleep
										// 2..停止 絶えずnp2stopemulateを監視
...
static	unsigned int drawskip = 0;

static	BOOL		dash = FALSE;		// ALT(R)キー押し下げによる高速実行
#endif

#if defined(SUPPORT_OPRECORD)
#define OPPLAYMAXFILES 10
static	char		opplayfile[OPPLAYMAXFILES][MAX_PATH];
static	int			opplayfiles = 0;	// opplayfileに格納されているファイルの数
static	int			opplayfilenext = 0;	// 次に再生するファイルのopplayfileでのindex
static	BOOL		oprepeat = FALSE;
static	BOOL		oprestart = FALSE;
#endif

#if defined(SUPPORT_PC88VA)
static const char np2help[] = "vaeg.chm";
#else
...
#if defined(VAEG_EXT)
static void np2core_update(void) {
	nowait = np2oscfg.NOWAIT || dash;
	drawskip = (dash) ? 16 : np2oscfg.DRAW_SKIP;
}
#endif
...
static void check_and_substitute_disk(char *path, UINT size, int *readonly, const char *defaultpath) {
	short	attr;
	FILEH	fh;
	char	str[MAX_PATH + 96];
	...
	if (path && path[0]) {
		for (;;) {
			...
			// filenameが有効でない。
			wsprintf(str, "Failed to access:\n  %s\nPlease select a substitute floppy image.", path);
			MessageBox(hWndMain, str, "Replay", MB_OK | MB_ICONWARNING);

			// 代わりのファイルを選択してもらう
			if (defaultpath) {
				file_cpyname(path, defaultpath, size);
			}
			else {
				path[0] = '\0';
			}
			if (dialog_selectfdimage(hWndMain, path, size, readonly)) {
			}
			else {
				// キャンセル
				path[0] = '\0';
...
static void opplay_statsave_load_disk_hook(int drv, char *path, UINT size, int *readonly) {
	check_and_substitute_disk(path, size, readonly, fdd_diskname(drv));
}

/*
static void opplay(const char *path) {
	...
	if (id == IDYES) {
		statsave_set_load_disk_hook(opplay_statsave_load_disk_hook);
		statsave_load(statpath);
		statsave_set_load_disk_hook(NULL);
		toolwin_setfdd(0, fdd_diskname(0));
		toolwin_setfdd(1, fdd_diskname(1));
	}
	sysmng_workclockreset();
	...
	if (id == IDYES) {
		...
		xmenu_setoprecord();
		file_cpyname(opplayfile, path, sizeof(opplayfile));
	}
...
//	BOOL	force = TRUE;
	...
	if (fh == FILEH_INVALID) {
		...
		statsave_skipall(fh);
		if (oprecord_check_version(fh, ver, sizeof(ver))) {
			// リリース不一致
			wsprintf(buf, 
				"This data is created by another version (%s).\nExecute it anyway ?", ver);
			id = MessageBox(hWndMain, buf, title,
	...
	if (id == IDYES) {
		ret = statsave_check(path, buf, sizeof(buf));
		if (ret & (~STATFLAG_DISKCHG)) {
			MessageBox(hWndMain, "Couldn't restart.\nThe state data is not compatible.", title, MB_OK | MB_ICONSTOP);
			id = IDNO;
		}
	//	else if ((!force) && (ret & STATFLAG_DISKCHG)) {
	//		char buf2[1024 + 256];
...
static void opplaymulti(void) {
	opplayfilenext = 0;
	opplay(opplayfile[0]);
}

static void oprestartcheck(void) {
	if (oprestart) {
		oprestart = FALSE;
		opplay(opplayfile[opplayfilenext]);
	}
}

static void playcompleted(void) {
	xmenu_setoprecord();
	opplayfilenext++;
	if (opplayfilenext < opplayfiles) {
		oprestart = TRUE;
	}
	else if (opplayfiles && oprepeat) {
		opplayfilenext = 0;
		oprestart = TRUE;
	}
}
...
/*
static void setfdd(UINT8 drv, const char *filename, int readonly) {
	short	attr;
	FILEH	fh;
	BOOL	badfile = TRUE;
	char	str[MAX_PATH + 96];
...
const char	*p;
	...
	if (filename) {
		while (badfile) {
			...
			wsprintf(str, "Failed to access:\n  %s\nPlease select a substitute floppy image.", filename);
			MessageBox(hWndMain, str, "Replay", MB_OK | MB_ICONWARNING);
			...
			if (dialog_selectfdimage(hWndMain, path, sizeof(path), &readonly)) {
				filename = path;
			}
			else {
				// キャンセル
	...
	diskdrv_setfdd(drv, filename, readonly);
	toolwin_setfdd(drv, filename);
}
*/
static void setfdd(UINT8 drv, const char *filename, int readonly) {
	char	path[MAX_PATH];
	...
	check_and_substitute_disk(path, sizeof(path), &readonly, fdd_diskname(drv));
...
/*
static void restartplay(void) {
	SendMessage(hWndMain, WM_COMMAND, IDM_OPRECORDPLAY + oprecord.playno, 0);
}
*/

static void oprec(const char *path) {
	int		ret;
	...
	if (ret) {
		file_delete(path);
		wsprintf(str, "Failed to save the state:\n  %s", path);
		MessageBox(hWndMain, str, "Record operation", MB_OK | MB_ICONSTOP);
	}
	else {
		fh = file_open(path);
		if (fh == FILEH_INVALID) {
...
static void np2cmd(HWND hWnd, UINT16 cmd) {
	...
	switch(cmd) {
...
#if defined(SUPPORT_PC88VA)
		case IDM_SOUNDBOARD2:
			xmenu_setsound(0x0200);
			update |= SYS_UPDATECFG;
			break;
#endif
		...
		case IDM_BMS:
			winuienter();
			DialogBox(hInst, MAKEINTRESOURCE(IDD_BMS),
...
#if defined(SUPPORT_PC88VA)
		case IDM_PORTJOY:
			xmenu_setmousedevice(MOUSEIFVA_JOYPAD);
			update |= SYS_UPDATECFG;
			break;

		case IDM_PORTMOUSE:
			xmenu_setmousedevice(MOUSEIFVA_MOUSE);
			update |= SYS_UPDATECFG;
			break;

		case IDM_VA91:
			xmenu_setva91(va91cfg.enabled ^ 1);
			update |= SYS_UPDATECFG;
			break;
#endif
		...
		case IDM_OPRECORDREPEAT:
			oprepeat = oprepeat ? FALSE : TRUE;
			xmenu_setrepeat(oprepeat);
			break;

		case IDM_OPRECORD_REC:
			{
				char	path[MAX_PATH];

				winuienter();
				b = dialog_oprec(hWnd, path, sizeof(path));
				winuileave();
				if (b) {
					oprec(path);
				}
...
/*
			winuienter();
			dialog_recoprecord(hWnd);
			xmenu_setoprecord();
			winuileave();
*/
			break;

		case IDM_OPRECORD_PLAY:
			winuienter();
			b = dialog_opplay(hWnd, opplayfile[0], sizeof(opplayfile[0]));
			winuileave();
...
/*
			{
				...
				winuienter();
				b = dialog_opplay(hWnd, path, sizeof(path));
				winuileave();
...
*/
		...
		case IDM_OPRECORD_PLAYMULTI:
			{
				int i;

				winuienter();
				b = TRUE;
				for (i = 0; i < OPPLAYMAXFILES; i++) {
					b = dialog_opplay(hWnd, opplayfile[i], sizeof(opplayfile[i]));
					if (!b) break;
				}
				winuileave();
...
#if defined(SUPPORT_OPRECORD)
/*
			if ((cmd >= IDM_OPRECORDREC) &&
				(cmd < (IDM_OPRECORDREC + SUPPORT_OPRECORD))) {
				char ext[4];
				char path[MAX_PATH];
				SPRINTF(ext, np2flagext, cmd - IDM_OPRECORDREC + 10);
				flagsave(ext);

				SPRINTF(ext, "O%02d", cmd - IDM_OPRECORDREC + 10);
				file_cpyname(path, modulefile, sizeof(path));
				file_cutext(path);
				file_catname(path, str_dot, sizeof(path));
				file_catname(path, ext, sizeof(path));
				oprecord_start_record(path);
			}
			if ((cmd >= IDM_OPRECORDPLAY) &&
				(cmd < (IDM_OPRECORDPLAY + SUPPORT_OPRECORD))) {
				char ext[4];
				char path[MAX_PATH];
				SPRINTF(ext, np2flagext, cmd - IDM_OPRECORDPLAY + 10);
				flagload(ext, "Status Load", TRUE);

				SPRINTF(ext, "O%02d", cmd - IDM_OPRECORDPLAY + 10);
				file_cpyname(path, modulefile, sizeof(path));
				file_cutext(path);
				file_catname(path, str_dot, sizeof(path));
				file_catname(path, ext, sizeof(path));
				oprecord_start_play(path);
				oprecord.playno = cmd - IDM_OPRECORDPLAY;
			}
*/
#endif
			break;
...
//				scrnmng_update();
		...
		case WM_KEYDOWN:
//			TRACEOUT(("WM_KEYDOWN: wParam = %02x lParam & 0x01000000 = %02x",wParam, lParam & 0x01000000));
			if (wParam == VK_F11) {
		...
		case WM_KEYUP:
//			TRACEOUT(("WM_KEYUP: wParam = %02x lParam & 0x01000000 = %02x",wParam, lParam & 0x01000000));
			if (wParam == VK_F11) {
		...
		case WM_SYSKEYDOWN:
//			TRACEOUT(("WM_SYSKEYDOWN: wParam = %02x lParam & 0x21000000 = %02x",wParam, lParam & 0x21000000));
...
#if defined(VAEG_EXT)
			if ((lParam & 0x1000000) && wParam == VK_MENU && np2oscfg.ALTRKEY == 9) {
				dash = TRUE;
				np2core_update();
				break;
			}
		...
		case WM_SYSKEYUP:
//			TRACEOUT(("WM_SYSKEYUP: wParam = %02x lParam & 0x21000000 = %02x",wParam, lParam & 0x21000000));
#if defined(VAEG_EXT)
			if ((lParam & 0x1000000) && wParam == VK_MENU && np2oscfg.ALTRKEY == 9) {
				dash = FALSE;
				np2core_update();
...
#endif
#if defined(SUPPORT_OPRECORD)
	oprecord_set_setfdd(setfdd);
//	oprecord_set_restartplay(restartplay);
	oprecord_set_playcompleted(playcompleted);
#endif
#if defined(SUPPORT_PC88VA)
	bkupmemva_load();
#endif
...
#if defined(VAEG_EXT)
	winkbd_setaltr(np2oscfg.ALTRKEY);
#endif
	keystat_initialize();
...
#if defined(VAEG_EXT)
	xmenu_setaltrkey(np2oscfg.ALTRKEY);
#endif
...
#if defined(SUPPORT_PC88VA)
	xmenu_setmousedevice(mouseifvacfg.device);
	xmenu_setva91(va91cfg.enabled);
#endif
...
#if defined(VAEG_EXT)
		soundmng_pcmload(SOUND_PCMSEEK, "seek.wav", EXTROMIO_FILE);
		soundmng_pcmload(SOUND_PCMSEEK1, "seek1.wav", EXTROMIO_FILE);
#else
...
#if defined(VAEG_EXT)
		soundmng_pcmload(SOUND_PCMHEADON, "headon.wav", EXTROMIO_FILE);
		soundmng_pcmload(SOUND_PCMHEADOFF, "headoff.wav", EXTROMIO_FILE);
		soundmng_pcmvolume(SOUND_PCMHEADON, np2cfg.MOTORVOL);
		soundmng_pcmvolume(SOUND_PCMHEADOFF, np2cfg.MOTORVOL);
#endif
	}
...
#if defined(VAEG_EXT)
	{
		...
		callback.wait = debugwait;
		callback.onpause = debugonpause;
		pccore_debugsetcallback(&callback);
	}
#endif
	S98_init();
