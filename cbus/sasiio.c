...
static void sasidmac(void) {
	...
	if ((sasiio.ocr & SASIOCR_DMAE) &&
	    ((sasiio.phase == SASIPHASE_READ) || (sasiio.phase == SASIPHASE_WRITE))) {
		en = TRUE;
	} else {
		en = FALSE;
	}
...
static void checkcmd(void) {
	...
	case 0x03: // Request Sense Status
		...
		sasiio.sens[1] = (BYTE)((sasiio.unit << 5) + ((sasiio.sector >> 16) & 0x1f));
...
REG8 DMACCALL sasi_dmafunc(REG8 func) {
	switch (func) {
	case DMAEXT_START:
		TRACEOUT(("sasi dma transfer!! %.5x:%.4x", dmac.dmach[0].adrs.d, dmac.dmach[0].leng.w));
		return (1);
