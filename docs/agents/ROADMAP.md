...
| M89 | tasks/M89_merge_va_source_directories.md | Consolidate the active VA BIOS, VRAM, and CPU adapter/backend directories into `bios/`, `vram/`, and `cpu/` without behavior change | **G89 human; implementation candidate [f93bb9a](https://github.com/nakatamaho/vaeg/commit/f93bb9a95b1236f977c671bd994e8de2a6e67ff5); validation in [M89 report](reports/m89_merge_va_source_directories.md)** |
...
-
  M89 source-directory consolidation is implemented on the candidate
  [f93bb9a](https://github.com/nakatamaho/vaeg/commit/f93bb9a95b1236f977c671bd994e8de2a6e67ff5).
  The rename-only, reference-fixup, and validation details are recorded in
  [`m89_merge_va_source_directories.md`](reports/m89_merge_va_source_directories.md).
  Local Linux Debug, Linux CI Clang/CTest, repository checks, and MinGW
  cross-build pass; G89 human validation and Hosted CI are pending.
  M89 is not merged to `main`.
