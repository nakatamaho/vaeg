...
| M72 | tasks/M72_misc_compile_flag_cleanup.md | Audit inactive VAEG-irrelevant code, remove only proven-safe inactive cleanup targets, remove About/More 98x1 UI details, fold always-enabled `VAEG_FIX`, and audit inactive PC-9821/EPSON/`CPUCORE_IA32`/IDE/PC-9861K/`VAEG_EXT`/font/embed boundaries without changing active behavior | **G72 human; do not declare passed** |
...
- **PC-88VA active-tree invariant (M15, updated by M72).** The active
  ...
  audits inactive IA32 CPU-core, IDE I/O, PC-9861K expansion serial, and other
  VAEG-irrelevant legacy code, and treats `VAEG_EXT` as obsolete active-tree
  cleanup rather than a feature to enable blindly when audit evidence proves
