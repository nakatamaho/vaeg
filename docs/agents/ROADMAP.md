...
| M60b | tasks/M60b_upd9002_rom_authority_epoch.md | Bind monitor-ROM and debugger authority to a content-addressed target-policy epoch and correct only the authorized 6C–6F and exact 0F gap classifications | **G60b pending; blocked on candidate review** |
| M60c | tasks/M60c_upd9002_fpo2_main_dispatch_audit.md | Audit the main dispatch and FPO2/66/67 target authority without changing instruction semantics | **G60c not started; predecessor approval required** |
| M60d | tasks/M60d_upd9002_interrupt_frame.md | Resolve only an independently proven synchronous interrupt-frame residual, or close with evidence if none remains | **G60d not started; predecessor approval required** |
| M60e | tasks/M60e_upd9002_iret.md | Correct only evidence-supported IRET semantics after interrupt-frame governance is closed | **G60e not started; predecessor approval required** |
| M61 | tasks/M61_upd9002_mov_immediate_register.md | Correct the C6/C7 register-form MOV-immediate family while leaving F7 `/2` for residue planning | **G61 not started; predecessor approval required** |
| M62a | tasks/M62a_upd9002_aam.md | Correct AAM from executed SST evidence while preserving D5 | **G62a not started; predecessor approval required** |
| M62b1 | tasks/M62b1_upd9002_ror4.md | Correct the target-authorized ROR4 family | **G62b1 not started; predecessor approval required** |
| M62b2 | tasks/M62b2_upd9002_rol4.md | Implement target-authorized ROL4 and govern its exact gap-to-applicable transition | **G62b2 not started; predecessor approval required** |
| M62c | tasks/M62c_upd9002_bcd_adjust.md | Correct the evidence-supported BCD and ASCII-adjust family | **G62c not started; predecessor approval required** |
| M63 | tasks/M63_upd9002_shift_semantics.md | Correct the shift family using evidence-derived count and FLAGS rules | **G63 not started; predecessor approval required** |
| M64 | tasks/M64_upd9002_div_idiv.md | Correct DIV and IDIV from executed SST evidence | **G64 not started; predecessor approval required** |
| M65 | tasks/M65_upd9002_residue_replan.md | Re-audit and re-plan the exact target-correct semantic residue | **G65 not started; predecessor approval required** |
| M66a | tasks/M66a_upd9002_drop_cpu286_state_compat.md | Remove obsolete CPU286 save-state compatibility only after generated semantic residue is closed | **G66a not started; predecessor approval required** |
| M66b | tasks/M66b_upd9002_remove_i286_identity.md | Remove the remaining active I286/i286c identity after state compatibility is closed | **G66b not started; predecessor approval required** |
| M67 | tasks/M67_upd9002_divergence_consolidation.md | Consolidate approved divergences, hardware questions, and the final target-policy evidence | **G67 not started; predecessor approval required** |

Phase 2 dependencies: M7 → M8 → {M9, M10 parallel} → M11 → M12 → M13.
Post-phase dependency: M13 → M14 → M15 → M16 → M17 → M18 → M19 → M20 → M21 → M22 → M23 → M24 → M25 → M26 → M27 → M28 → M29 → M30 → M31 → M32. The required Z80 migration sequence M34 → M35 → M36 → M37 → M38 → M39 → M40 → M41 is complete. The separately authorized uPD9002 preparation sequence passed G42 through G51. M52–M56 were consumed by unrelated work and retain their historical meanings. The renumbered semantics campaign passed G57 at exactly `72322d5c9b8e40e4a988312aebe163a8190e2aa5`, G58 at exactly `bc8a55c6da1082b85b794068e0d933e31fe46b13`, G59 at exactly `e7f2325bc81310532091a8ca82914030fdb8b6ba`, and G60a at exactly `ba2b7d3f5c76646b30d63fd8951f4a1964817b15`. The v5 prospective sequence is M60b → M60c → M60d → M60e → M61 → M62a → M62b1 → M62b2 → M62c → M63 → M64 → M65 → M66a → M66b → M67; every item stops at its own gate. See [`UPD9002_SEMANTICS_MIGRATION.md`](UPD9002_SEMANTICS_MIGRATION.md). The accepted M47 pre-implementation audit disproved the old assumption that REP-prefixed 0x0F could not reach NP2 80286 system handlers. M47 therefore collected correctness evidence, M48 installed the approved fail-closed policy, and M49/M50 audited and removed only the explicitly approved dependency-closed groups.
