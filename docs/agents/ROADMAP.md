...
| M56 | tasks/M56_hostfs_readonly_redirector.md | Add a clean-room two-clause-BSD read-only DOS redirector with live file-level host-directory access | **G56 pending** |
...
M56 adds an independently authored read-only DOS network-redirector client
and bounded emulator-private file RPC. `HOSTFS.COM` exposes file-level host
directory access without FAT/BPB/sector emulation, while all host mutations
fail closed. HOSTFAT remains supported for compatibility but is hidden from
the normal Configure UI when HOSTFS is available. The clean-room boundary,
state policy, and G56 checklist are in
