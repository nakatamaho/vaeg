...
static const char *cfgstr[] = {str_dir, str_source, str_default, str_bank, str_drumset};
	...
	for (i = 2; i < argc; i++) {
		...
		*data++ = '\0';
		if (!milstr_cmp(key, str_amp)) {
			...
			if ((cfggetval(data, &val) == SUCCESS) && (val >= 0) && (val < 128)) {
				tone->note = (BYTE)val;
			}
...
BOOL cfgfile_getfile(MIDIMOD mod, const char *filename, char *path, int size) {
	PATHLIST p;
	short attr;

	if ((filename == NULL) || (filename[0] == '\0') || (path == NULL) || (size == 0)) {
		goto fpgf_exit;
	}
	p = mod->pathlist;
	while (p) {
...
BOOL cfgfile_load(MIDIMOD mod, const char *filename, int depth) {
	...
	if ((depth >= 16) || (cfgfile_getfile(mod, filename, buf, sizeof(buf)) != SUCCESS)) {
		goto cfl_err;
	}
	//	TRACEOUT(("open: %s", buf));
	...
	while (textfile_read(tfh, buf, sizeof(buf)) == SUCCESS) {
		...
		case CFG_BANK:
		case CFG_DRUM:
			if ((cfggetval(argv[1], &val) == SUCCESS) && (val >= 0) && (val < 128)) {
				val <<= 1;
				if (cfg == CFG_DRUM) {
					val++;
				}
