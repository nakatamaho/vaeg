...
static const int gaintbl[24 + 1] = {16,  19,  22,  26,  32,  38,  45,  53,  64,  76,  90,  107, 128,
                                    152, 181, 215, 256, 304, 362, 430, 512, 608, 724, 861, 1024};
...
static void voice_on(MIDIHDL midi, CHANNEL ch, VOICE v, int key, int vel) {
	...
	if (!layer->samprate) {
		...
		v->freq = (float)layer->samprate / (float)midi->samprate * (float)v->frequency /
		          (float)layer->freqroot;
	}
...
static void key_on(MIDIHDL midi, CHANNEL ch, int key, int vel) {
	...
	do {
		...
		if (v2->phase == VOICE_FREE) {
			v = v2;
		} else if ((v2->channel == ch) && ((v2->note == key) || (ch->flag & CHANNEL_MONO))) {
...
static void key_off(MIDIHDL midi, CHANNEL ch, int key) {
	...
	vterm = v + VOICE_MAX;
	do {
		if ((v->phase & VOICE_ON) && (v->channel == ch) && (v->note == key)) {
...
static void pitchbendor(MIDIHDL midi, CHANNEL ch, int val1, int val2) {
	...
	if (1) {
		...
		if (!val1) {
			...
			val1 *= ch->pitchsens;
			ch->pitchfactor = bendhtbl[(val1 >> (6 + 7)) + 24] * bendltbl[(val1 >> 7) & 0x3f];
		}
		v = midi->voice;
		vterm = v + VOICE_MAX;
		do {
...
static void longmsg_roland(MIDIHDL hdl, const BYTE *msg, UINT size) {
	...
	if ((size > 10) && (msg[2] == 0x10) && (msg[3] == 0x42) && (msg[4] == 0x12)) { // GS data set
