...
static BOOL msa_open(GETSND snd, WAVE_INFOS *wavehead, UINT headsize) {
	...
	if ((snd->bit != 4) || (headsize < (sizeof(WAVE_INFOS) + sizeof(WAVE_MSA_INFO)))) {
		goto msaopn_err;
	}
	...
	blk -= 6;
	blk *= 2;
	TRACEOUT(("wav: msa: ExtraSize %d / SPB=%d NumOfCoefs=%d", exsize, spb, numcoef));
...
BOOL getwave_open(GETSND snd, BYTE *ptr, UINT size) {
	...
	if (!memcmp(riff->fmt, fmt_wave, 4)) {
		...
		TRACEOUT(
		    ("wav: fmt: %x / %dch %dHz %dbit", format, snd->channels, snd->samplingrate, snd->bit));
