...
static const char *flagstr[16][2] = {{NULL, NULL},  // 0x8000
                                     {NULL, NULL},  // 0x4000
                                     {NULL, NULL},  // 0x2000
                                     {NULL, NULL},  // 0x1000
                                     {s_nv, s_ov},  // 0x0800
...
static const char str_register[] =
    ...
    "SP=%.4x  BP=%.4x  SI=%.4x  DI=%.4x" CRLITERAL "DS=%.4x  ES=%.4x  SS=%.4x  CS=%.4x  "
    "IP=%.4x   ";
static const char str_picstat[] =
    CRLITERAL "PIC0=%.2x:%.2x:%.2x" CRLITERAL "PIC1=%.2x:%.2x:%.2x" CRLITERAL
              "8255PORTC = %.2x / system-port = %.2x";
...
const char *debugsub_regs(void) {
	...
	SPRINTF(work, str_register, CPU_AX, CPU_BX, CPU_CX, CPU_DX, CPU_SP, CPU_BP, CPU_SI, CPU_DI,
